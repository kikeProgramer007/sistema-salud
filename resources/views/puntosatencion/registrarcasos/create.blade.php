<x-app-layout>
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('registrarcasosPunto.index') }}"
                        ...
                        Lista de Casos Puntos de Atencion
                    </a>
                </div>
            </li>
    ...
    <div class="mt-2">
        ...
        <form method="POST" action="{{ route('registrarcasosPunto.store') }}">
            @csrf
            <div
                ...
                <div class="grid grid-cols-6 gap-6">
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        <label for="punto_atencion_id"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Puntos de Atención</label>
                        <select id="punto_atencion_id" name="punto_atencion_id"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            @foreach ($puntosAtenciones as $puntosAtencion)
                                <option value="{{ $puntosAtencion->id }}">{{ $puntosAtencion->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
    ...
    <x-views>
        Vistas: {{ getPageViews('caso_hospital_crear') }}
    </x-views>
    @push('scripts')
