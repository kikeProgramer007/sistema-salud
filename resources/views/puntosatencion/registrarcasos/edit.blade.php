<x-app-layout>
    ...
    <div class="mt-2">
        ...
        <form method="POST" action="{{ route('registrarcasosPunto.update', $caso) }}">
            @csrf
            @method('put')
            <div
                ...
                <div class="grid grid-cols-6 gap-6">
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <select id="user_id" name="user_id" required
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}" {{ $user->id == $caso->user_id ? 'selected' : '' }}>
                                    {{ $user->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select id="estado_id" name="estado_id" required
                            ...
                            @foreach ($estados as $estado)
                                <option value="{{ $estado->id }}"
                                    {{ $estado->id == $caso->estado_id ? 'selected' : '' }}>
                                    {{ $estado->estado }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <select id="enfermedad_id" name="enfermedad_id" required
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            oninvalid="this.setCustomValidity('Selecciona al menos un campo')"
                            oninput="this.setCustomValidity('')">
                            @foreach ($enfermedades as $enfermedad)
                                <option value="{{ $enfermedad->id }}"
                                    {{ $enfermedad->id == $caso->enfermedad_id ? 'selected' : '' }}>
                                    {{ $enfermedad->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <select id="punto_atencion_id" name="punto_atencion_id" required
                            ...
                            oninvalid="this.setCustomValidity('Selecciona al menos un campo')"
                            oninput="this.setCustomValidity('')">
                            @foreach ($puntosAtenciones as $puntosAtencion)
                                <option value="{{ $puntosAtencion->id }}"
                                    {{ $puntosAtencion->id == $caso->estadia_enfermedable_id ? 'selected' : '' }}>
                                    {{ $puntosAtencion->nombre }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <textarea name="detalle" id="detalle" rows="4"
                            ...
                            placeholder="Ingrese detalles del registros">{{ old('detalle', $caso->detalle) }}</textarea>
                    </div>
                        ...
                        <select id="sintoma_id" multiple name="sintoma_id[]" required
                            ...
                            @foreach ($sintomas as $sintoma)
                                <option value="{{ $sintoma->id }}"
                                    {{ in_array($sintoma->id, $caso->sintomas->pluck('id')->toArray()) ? 'selected' : '' }}>
                                    {{ $sintoma->nombre }}
                                </option>
                            @endforeach
                        </select>
    ...
    <x-views>
        Vistas: {{ getPageViews('caso_hospital_editar') }}
    </x-views>
    @push('scripts')
