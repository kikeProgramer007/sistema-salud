<x-app-layout>
    ...
    <div class="mt-2">
        ...
        <form method="POST" action="{{ route('solicitudes.update', $solicitude) }}" onsubmit="validate()">
            @method('put')
            @csrf
            <div
                ...
                <div class="grid grid-cols-6 gap-6">
                        ...
                        <select id="punto_atencion_id" name="punto_atencion_id" required
                            ...
                            @foreach ($puntos as $punto)
                                <option value="{{ $punto->id }}"
                                    {{ $punto->id == $puntoSolicitado ? 'selected' : '' }}>
                                    {{ $punto->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-span-6 sm:col-full">
                        ...
                        <button onclick="window.location.href = '{{ route('prediccion.index') }}'"
                            class="ml-4 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                            type="button">Analizar Situación</button>
                    </div>
                </div>
    ...
    <x-views>
        Vistas: {{ getPageViews('solicitudes_editar') }}
    </x-views>
    @push('scripts')
