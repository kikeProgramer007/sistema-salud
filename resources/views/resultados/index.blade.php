<x-app-layout>
    ...
    <div class="mt-3">
        ...
        <div class="items-center justify-between block sm:flex pb-4">
            ...
            <div class="flex items-center ml-auto space-x-2 sm:space-x-3">
                ...
                <button id="createProductButton" onclick="window.location.href = '{{ route('atenciones.index') }}'"
                    class="inline-flex items-center justify-center w-1/2 px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 sm:w-auto dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                    type="button" data-drawer-target="drawer-create-product-default"
                    data-drawer-show="drawer-create-product-default" aria-controls="drawer-create-product-default"
                    data-drawer-placement="right">
                    Ir a ista de Atenciones
                </button>
            </div>
        </div>
                ...
                <tbody>
                    @foreach ($resultados as $resultado)
                        <tr
                            ...
                            <td class="px-6 py-4">
                                {{ $resultado->atencion->medico->name }}
                            </td>
                            <td class="px-6 py-4">
                                {{ $resultado->atencion->paciente->name }}
                            </td>
                            <td class="px-6 py-4">
                                <a href="{{ route('resultados.show', $resultado) }}"
                                    class="mr-2 font-medium text-green-600 dark:text-green-500 hover:underline">Ver</a>
                                <a href="#" onclick="event.preventDefault(); confirmDelete({{ $resultado->id }})"
                                    class="font-medium text-red-600 dark:text-red-500 hover:underline">Eliminar</a>

                                <form id="delete-form-resultado-{{ $resultado->id }}"
                                    action="{{ route('resultados.destroy', $resultado) }}" method="POST"
                                    style="display: none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
    ...
    <x-views>
        Vistas: {{ getPageViews('resultados_crear') }}
    </x-views>
    @push('scripts')
        <script>
            function confirmDelete(formId) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        setTimeout(function() {
                            document.getElementById('delete-form-resultado-' + formId).submit();
                        }, 1000);
                    }
                });
            }
