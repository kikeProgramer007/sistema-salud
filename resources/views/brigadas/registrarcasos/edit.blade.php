<x-app-layout>
    ...
    <div class="mt-2">
        ...
        <form method="POST" action="{{ route('registrarcasosBrigada.update', $caso) }}">
            @csrf
            @method('put')
            <div
                ...
                <div class="grid grid-cols-6 gap-6">
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <select id="brigada_id" name="brigada_id" required
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            oninvalid="this.setCustomValidity('Selecciona al menos un campo')"
                            oninput="this.setCustomValidity('')">
                            @foreach ($brigadas as $brigada)
                                <option value="{{ $brigada->id }}"
                                    {{ $brigada->id == $caso->estadia_enfermedable_id ? 'selected' : '' }}>
                                    {{ $brigada->name }}
                                </option>
                            @endforeach
                        </select>
    ...
    <x-views>
        Vistas: {{ getPageViews('caso_brigada_editar') }}
    </x-views>
    @push('scripts')
